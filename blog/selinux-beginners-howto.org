...
** Режимы SELinux
 ...
 - <<<enforcing>>>: включен, правила блокируют, сообщения о блокировке
   уходят в ~/var/log/audit/audit.log~
 - <<<permissive>>>: ничего не блокируется, только уходят сообщения в
...
Каждый объект мира (файл, процесс и т.д.) имеют свойство <<<context>>>. В
...
Каждый контекст состоит из трех частей:
 1. <<<user>>>
 2. <<<role>>>
 3. <<<type>>> (у процессов его принято называть <<<domain>>>, но разницы никакой нет)
...
<<<make reload>>> Загрузить заново после пересборки:
...
Теперь можно поправить модуль, затем заново его [[make reload][загрузить]].
